...
for name in names:         
    for i in range(1,2):    
        ...
        subprocess.run(f'echo "protein" | gmx editconf -f {i}_{name}.pdb -o {i}_{name}_box.pdb -c -d 1.2 -princ', shell=True)                             ##. !!修改02 盒子大小
        ...
        subprocess.run(f'echo "sol\n" | gmx genion -s {i}_{name}_ions.tpr -o {i}_{name}_ions.pdb -neutral -p {i}_{name}_topol.top', shell=True) 
